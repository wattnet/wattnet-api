"""Integration tests for GET /v1/mix_share."""
...
def _m(**kw) -> FakeMetric:
    d = {"zone": "ES", "unit": "%", "source": "FR",
         "valid": "true", "zone_status": "complete"}
    d.update(kw)
    return FakeMetric(_NOW, 20.0, d)
...
def test_returns_mix_share_structure(client, mock_db) -> None:
    """Valid metric produces a correctly nested MixShare object.
    ...
    assert body[0]["series"][0]["shares"][0]["origin"] == "FR"
...
def test_origin_zone_and_origin_lat_returns_400(client, mock_db) -> None:
    """Providing both origin_zone and origin_lat must return 400.

    :return: None
    :rtype: None
    """
    mock_db([])
    assert client.get(f"{_URL}?origin_zone=FR&origin_lat=48").status_code == 400
