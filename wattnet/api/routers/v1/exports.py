...
@router.get(
    ...
    description="""
Retrieve a list of export data filtered by:
...
def get_exports(
    ...
    destination_zone: Optional[str] = Query(
        None,
        description="Destination zone code for exports (mutually exclusive with destination_lat/destination_lon)",
    ),
    destination_lat: Optional[float] = Query(
        None, ge=-90, le=90, description="Latitude for destination zone"
    ),
    destination_lon: Optional[float] = Query(
        None, ge=-180, le=180, description="Longitude for destination zone"
    ),
